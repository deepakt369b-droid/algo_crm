...
- Look for actual company websites with real business information`,
      ...
      if (choice.message.refusal) {
        throw new Error(`OpenAI extraction refused: ${choice.message.refusal}`);
      }
...
7. Common fake URL patterns to AVOID:
   ...
   - Any URL with brackets or placeholders`
      ...
      const choice = response.choices[0];
      if (choice.message.refusal) {
        throw new Error(`OpenAI corroborated extraction refused: ${choice.message.refusal}`);
      }
...
**FORBIDDEN**: Do not use general knowledge or make educated guesses. Only extract what is ACTUALLY WRITTEN in the prompt.`
      ...
      const choice = response.choices[0];
      if (choice.message.refusal) {
        throw new Error(`OpenAI structured extraction refused: ${choice.message.refusal}`);
      }
...
Generate new search queries to find: ${targetField}`,
      ...
      const choice = response.choices[0];
      if (choice.message.refusal) {
        throw new Error(`OpenAI query generation refused: ${choice.message.refusal}`);
      }
...
Return ONLY the search query, nothing else.`
      ...
      const response = await this.client.chat.completions.create({
        ...
        temperature: 0.3,
        max_tokens: 100,
        user: (context?.userId as string) || "system",
      });

      const choice = response.choices[0];
      if (choice.message.refusal) {
        throw new Error(`OpenAI search query generation refused: ${choice.message.refusal}`);
      }

      return choice.message.content?.trim() || question;
    } catch (error) {
...
- Be specific with your answer, citing actual data values`
      ...
      const choice = response.choices[0];
      if (choice.message.refusal) {
        throw new Error(`OpenAI table data analysis refused: ${choice.message.refusal}`);
      }
  ...
  async selectBestSource(
    ...
    try {
      ...
      const choice = response.choices[0];
      if (choice.message.refusal) {
        throw new Error(`OpenAI source selection refused: ${choice.message.refusal}`);
      }

      const result = JSON.parse(choice.message.content || '{"index": 0}');
...
Be concise but informative. If the content doesn't contain the answer, say so. When citing dates or time-sensitive information, consider the current date for context.`
      ...
      const choice = response.choices[0];
      if (choice.message.refusal) {
        throw new Error(`OpenAI conversational response refused: ${choice.message.refusal}`);
      }

      return choice.message.content?.trim() || "I couldn't find a clear answer to your question.";
    } catch (error) {
