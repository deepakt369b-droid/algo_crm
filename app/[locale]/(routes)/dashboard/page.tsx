...
const DashboardPage = async () => {
  ...
  const [
    ...
    getLeadsCount().catch(() => 0),
    getTasksCount().catch(() => 0),
    getInvoicesCount().catch(() => 0),
    getCampaignsCount().catch(() => 0),
    getTargetsCount().catch(() => 0),
    getStorageSize().catch(() => 0),
    getBoardsCount().catch(() => 0),
    getContactCount().catch(() => 0),
    getContractsCount().catch(() => 0),
    getActiveUsersCount().catch(() => 0),
    getAccountsCount().catch(() => 0),
    getExpectedRevenue(displayCurrency).catch(() => 0),
    getDocumentsCount().catch(() => 0),
    getOpportunitiesCount().catch(() => 0),
    getUsersTasksCount(userId).catch(() => 0),
  ]);
