...
export default async function EnrichmentJobsPage() {
  ...
  const records = (await supabaseAdmin.from("crm_Contact_Enrichment").select("*, contact(id, first_name, last_name, email), triggered_by_user(name)").order("createdAt", { ascending: false }).limit(200)).data;
