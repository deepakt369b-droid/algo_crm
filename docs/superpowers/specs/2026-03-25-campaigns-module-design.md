...
A new top-level **Campaigns** module for Flowline Pro that enables users to create, schedule, and track email marketing campaigns. Targets and Target Lists are moved here from the CRM module. Campaign templates are created with AI generation (using existing LLM keys) and refined in a TipTap WYSIWYG editor. Campaigns are sent via Resend, scheduled via Inngest, and tracked with delivery + engagement analytics (opens, clicks, bounces).
