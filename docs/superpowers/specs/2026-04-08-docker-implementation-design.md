...
Enable users to clone the Flowline Pro repo and run `docker-compose up` to get a fully working instance — app, database, background jobs, and file storage — with zero manual configuration. Optimized for self-hosting platforms like Coolify.

## User Experience

```bash
git clone https://github.com/pdovhomilja/flowlinepro-app
cd flowlinepro-app
docker-compose up
...
|---------|-------|--------------|--------------|---------|
| app | Built from `./Dockerfile` | 3000 | 3000 | Flowline Pro application |
...
- Single Docker network: `flowlinepro`
...
3. **Create MinIO bucket** — ensure the `flowlinepro` bucket exists using the MinIO S3 API via `curl` (PUT request to create bucket). No additional tools needed. Idempotent — skips if bucket already present (ignores 409 BucketAlreadyOwnedByYou).
...
| Service | Credentials |
|---------|------------|
| Postgres | user: `flowlinepro`, password: `flowlinepro`, database: `flowlinepro` |
| MinIO | access key: `minioadmin`, secret key: `minioadmin123`, bucket: `flowlinepro` |
...
|---------|-------|----------|---------|
| postgres | `pg_isready -U flowlinepro` | 5s | 5 |
...
| File | Purpose |
|------|---------|
| `Dockerfile` | Multi-stage build for Flowline Pro |
