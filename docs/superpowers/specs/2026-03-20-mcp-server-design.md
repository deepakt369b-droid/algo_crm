# Flowline Pro MCP Server — Design Spec
...
Add an MCP (Model Context Protocol) server to Flowline Pro so AI agents can operate on CRM data (Accounts, Leads, Opportunities, Contacts, Targets) via natural language. Users generate Bearer tokens in their profile page to authenticate MCP clients.
