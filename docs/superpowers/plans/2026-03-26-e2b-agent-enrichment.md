...
```bash
npx e2b auth login
npx e2b template build --name flowlinepro-enrichment
```

Expected output includes a template ID like `flowlinepro-enrichment-abc123`. Copy this ID.
...
```
E2B_API_KEY=e2b_***
E2B_ENRICHMENT_TEMPLATE=flowlinepro-enrichment
```
  ...
  async ({ event, step }) => {
    ...
    const agentOutput = await step.run("run-e2b-agent", async (): Promise<AgentOutput> => {
      ...
      const sandbox = await Sandbox.create({
        template: process.env.E2B_ENRICHMENT_TEMPLATE ?? "flowlinepro-enrichment",
