...
**Goal:** Enable users to run `docker-compose up` and get a fully working Flowline Pro instance with app, Postgres, Inngest, and MinIO.
...
echo "==> Flowline Pro Docker Entrypoint"
...
```bash
docker build -t flowlinepro:test .
```

Expected: Build completes successfully. If Prisma COPY paths don't resolve, inspect the build stage to find exact paths:

```bash
docker build --target build -t flowlinepro:build-debug .
docker run --rm flowlinepro:build-debug ls -la node_modules/.pnpm/ | grep prisma
```
...
```bash
git add Dockerfile
git commit -m "feat: add multi-stage Dockerfile for Flowline Pro"
```
...
services:
  ...
  postgres:
    ...
    environment:
      POSTGRES_USER: flowlinepro
      POSTGRES_PASSWORD: flowlinepro
      POSTGRES_DB: flowlinepro
    volumes:
      - postgres_data:/var/lib/postgresql/data
    networks:
      - flowlinepro
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U flowlinepro"]
      interval: 5s
      timeout: 5s
      retries: 5
  ...
  app:
    ...
    environment:
      # Database
      DATABASE_URL: postgresql://flowlinepro:flowlinepro@postgres:5432/flowlinepro
      DB_HOST: postgres
      DB_PORT: "5432"
      DB_USER: flowlinepro
      ...
      # Inngest
      INNGEST_DEV: "1"
      INNGEST_ID: flowlinepro
      INNGEST_APP_NAME: Flowline Pro
...
```bash
# ===========================================
# Flowline Pro Docker Environment Configuration
# ===========================================
...
# DATABASE_URL=postgresql://user:pass@your-host:5432/flowlinepro
...
6. `==> Ensuring MinIO bucket 'flowlinepro' exists...`
7. `==> Bucket 'flowlinepro' created.`
8. `==> No users found, seeding database...`
9. `==> Seeding complete.`
10. `==> Starting Flowline Pro...`
...
```bash
docker-compose down -v
docker rmi flowlinepro-app-app 2>/dev/null || true
docker-compose up --build -d
```
